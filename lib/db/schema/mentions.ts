...
export const insertMentionParams = baseSchema.extend({
...
export const updateMentionParams = updateMentionSchema.extend({
  commentId: z.coerce.string().min(1)
}).omit({ 
  userId: true
});
...
export type CompleteMention = Awaited<ReturnType<typeof getMentions>>["mentions"][number];
