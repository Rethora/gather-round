...
export const insertRsvpParams = baseSchema.extend({
...
export const updateRsvpParams = updateRsvpSchema.extend({
  eventId: z.coerce.string().min(1)
}).omit({ 
  userId: true
});
...
export type CompleteRsvp = Awaited<ReturnType<typeof getRsvps>>["rsvps"][number];
