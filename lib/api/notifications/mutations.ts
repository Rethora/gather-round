...
export const updateNotification = async (id: NotificationId, notification: UpdateNotificationParams) => {
  ...
  try {
    const n = await db.notification.update({ where: { id: notificationId, userId: session?.user.id! }, data: newNotification})
    return { notification: n };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
    console.error(message);
...
export const deleteNotification = async (id: NotificationId) => {
  ...
  const { id: notificationId } = notificationIdSchema.parse({ id });
  try {
    const n = await db.notification.delete({ where: { id: notificationId, userId: session?.user.id! }})
    return { notification: n };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
    console.error(message);
