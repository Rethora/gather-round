...
export const updateEvent = async (id: EventId, event: UpdateEventParams) => {
  ...
  try {
    const e = await db.event.update({ where: { id: eventId, userId: session?.user.id! }, data: newEvent})
    return { event: e };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
    console.error(message);
...
export const deleteEvent = async (id: EventId) => {
  ...
  const { id: eventId } = eventIdSchema.parse({ id });
  try {
    const e = await db.event.delete({ where: { id: eventId, userId: session?.user.id! }})
    return { event: e };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
    console.error(message);
