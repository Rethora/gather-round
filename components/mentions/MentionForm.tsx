...
const MentionForm = ({
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    const mentionParsed = await insertMentionParams.safeParseAsync({ commentId, ...payload });
    ...
    const pendingMention: Mention = {
      updatedAt: mention?.updatedAt ?? new Date(),
    ...
    <form action={handleSubmit} onChange={handleChange} className={"space-y-8"}>
      {/* Schema fields start */}
        ...
        <Label
          className={cn(
            "mb-2 inline-block",
            errors?.mentionedUser ? "text-destructive" : "",
          )}
        >
          Mentioned User
        </Label>
        <Input
          type="text"
          name="mentionedUser"
          className={cn(errors?.mentionedUser ? "ring ring-destructive" : "")}
          defaultValue={mention?.mentionedUser ?? ""}
        />
        {errors?.mentionedUser ? (
          <p className="text-xs text-destructive mt-2">{errors.mentionedUser[0]}</p>
        ) : (
          <div className="h-6" />
        )}
      </div>
        ...
        <Select defaultValue={mention?.commentId} name="commentId">
          ...
          {comments?.map((comment) => (
            <SelectItem key={comment.id} value={comment.id.toString()}>
              {comment.id}{/* TODO: Replace with a field from the comment model */}
            </SelectItem>
           ))}
          </SelectContent>
        </Select>
      ...
      {editing ? (
        <Button
          ...
          onClick={() => {
            ...
            closeModal && closeModal();
            startMutation(async () => {
              addOptimistic && addOptimistic({ action: "delete", data: mention });
